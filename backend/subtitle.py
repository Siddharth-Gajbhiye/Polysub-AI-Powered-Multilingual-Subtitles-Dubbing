...
def translate_segments_nllb_batched(
    ...
    """
    Improved translation with:
      - Sentence-boundary merging
      - Context-aware full sentence translation
      - Proportional redistribution
      - Post-cleanup with tidy_translated_segments
    """
    translator = pipeline(
        "translation",
        model=nllb_model,
    ...
    # Merge into sentence-level blocks
    sentence_blocks = _merge_by_sentence_boundaries(segments)

    translated_blocks = []
    for block in sentence_blocks:
        try:
            block_translation = translator(block["text"], max_length=max_length)[0]["translation_text"].strip()
        except Exception as e:
            print(f"[WARN] Sentence translation failed → fallback: {e}")
            block_translation = block["text"]

        translated_blocks.append({
            "start": block["start"],
            "end": block["end"],
            "text": block_translation
        })

    # Redistribute back into original segments
    translated = []
    b_idx, t_idx = 0, 0
    while b_idx < len(sentence_blocks):
        ...
        total_chars = sum(len(s["text"].strip()) for s in segs_in_block)
        offset = 0
        for seg in segs_in_block:
            portion = len(seg["text"].strip()) / total_chars if total_chars > 0 else 0
            count = max(1, int(round(len(block_tokens) * portion)))
            slice_tokens = block_tokens[offset: offset + count]
            translated.append({
                "start": seg["start"],
                "end": seg["end"],
    ...
    # ✅ Post-clean translation cleanup
    translated = tidy_translated_segments(translated)
    return translated

def localize_numbers(text: str, translator, tgt_lang_nllb: str) -> str:
    """
    Convert numbers in text into target-language words using NLLB.
    Example: 'I have 25 apples' -> 'मेरे पास पच्चीस सेब हैं' (for Hindi).
    """
    def replace_num(match):
        num_str = match.group(0)
        try:
            # send just the number to NLLB
            translated_num = translator(num_str, max_length=32)[0]['translation_text'].strip()
            return translated_num if translated_num else num_str
        except Exception:
            return num_str

    return re.sub(r"\d+", replace_num, text)

def translate_srt_file_segment(
    input_srt, 
    output_srt, 
    src_lang_nllb, 
    tgt_lang_nllb
    ...
    """
    Improved SRT translation with:
      - Sentence-boundary merging
      - Context-aware full sentence translation
      - Duration-aware redistribution
      - Last-event absorb safeguard
      - Post-cleanup with tidy_translated_segments
    """
    subs = pysubs2.load(input_srt, encoding="utf-8")
    ...
    for block in merged_blocks:
        ...
        try:
            translated_text = translator(block_text, max_length=512)[0]['translation_text'].strip()
        except Exception as e:
            print(f"[WARN] Sentence translation failed → fallback: {e}")
    ...
    # === Redistribute into original events (duration + chars) ===
    translated_events = []
    for block, trans_block in zip(merged_blocks, translated_blocks):
        block_tokens = trans_block["text"].split()
        events = block["events"]

        # weight = (chars+1) × duration
        total_weight = sum(
            (len(ev.text.strip()) + 1) * max((ev.end - ev.start), 1)
            for ev in events
        )

        offset = 0
        for i, ev in enumerate(events):
            weight = (len(ev.text.strip()) + 1) * max((ev.end - ev.start), 1)
            portion = weight / total_weight if total_weight > 0 else 0
            count = max(1, int(round(len(block_tokens) * portion)))

            # last event gets all remaining tokens
            if i == len(events) - 1:
                slice_tokens = block_tokens[offset:]
            else:
                slice_tokens = block_tokens[offset: offset + count]

            ev.text = " ".join(slice_tokens).strip()
            translated_events.append(ev)
            offset += count

    # === Cleanup (dedupe + boundary fix) ===
    translated_events = tidy_translated_segments([
        {"start": ev.start/1000, "end": ev.end/1000, "text": ev.text}
        for ev in translated_events
    ])

    # Convert back to SSAEvent for saving
    final_events = []
    ...
    ssa.save(output_srt, format_="srt", encoding="utf-8")
    print(f"[IMPROVED-SENTENCE+DURATION+LAST-ABSORB] Final translated SRT → {output_srt}")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for tgt_lang in args.target_langs:
            ...
            # --- Translation (using translate_srt_file_segment) ---
            ...
            print(f"[SRT-TRANSLATE] Final translated SRT → {out_srt_final}")
